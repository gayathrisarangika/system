            ...
            <header className="bg-white border-b py-6">
                <div className="container mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-6">
                    <div className="flex-1 text-center md:text-left">
                        <h1 className="text-2xl md:text-3xl font-serif font-bold text-blue-900">{journal.journal_title}</h1>
                        <p className="text-md text-gray-600 mt-1 uppercase tracking-wide">{journal.university_name}</p>
                    </div>
                </div>
            </header>
                ...
                <div className="flex flex-col lg:flex-row gap-10">
                    ...
                    <div className="flex-1 bg-white p-8 rounded-lg shadow-sm border border-gray-200">
                        ...
                        <header className="mb-10">
                            ...
                            <div className="flex flex-wrap gap-4 text-lg text-blue-700 font-medium mb-6">
                                {article.author.split(',').map((author, index) => (
                                    <span key={index} className="hover:underline cursor-pointer">{author.trim()}</span>
                                ))}
                            </div>

                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-6 border-y border-gray-100 text-sm text-gray-600">
                                <div>
                                    <p><span className="font-bold">DOI:</span> 10.18535/ijsrm/v{article.issue.volume}i{article.issue.issue}.{article.id}</p>
                                    <p><span className="font-bold">Pages:</span> 1-10 (Sample)</p>
                                </div>
                                <div>
                                    <p><span className="font-bold">Published:</span> {new Date(article.created_at).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</p>
                        ...
                        <section className="mb-10">
                            ...
                            <div className="prose max-w-none text-gray-700 leading-relaxed font-serif text-lg whitespace-pre-wrap">
                                {article.abstract}
                            </div>
                        </section>
                    ...
                    <aside className="lg:w-80 flex flex-col gap-8">
                        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
                            <h4 className="text-lg font-bold text-blue-900 mb-4 border-b pb-2">How to Cite</h4>
                            <div className="text-sm text-gray-600 bg-gray-50 p-4 rounded border border-gray-100 font-serif">
                                {article.author.split(',')[0].trim()} ({article.year}). {article.title}. {journal.journal_title}, {article.issue.volume}({article.issue.issue}).
                            </div>
                            <button className="mt-4 text-blue-700 text-sm font-bold hover:underline flex items-center gap-1">
                        ...
                        <div className="bg-blue-900 text-white p-6 rounded-lg shadow-md">
                            ...
                            <div className="flex gap-4">
                                {/* Sample icons */}
                                <div className="w-10 h-10 bg-blue-800 rounded-full flex items-center justify-center hover:bg-blue-700 cursor-pointer transition">F</div>
                                <div className="w-10 h-10 bg-blue-800 rounded-full flex items-center justify-center hover:bg-blue-700 cursor-pointer transition">X</div>
                                <div className="w-10 h-10 bg-blue-800 rounded-full flex items-center justify-center hover:bg-blue-700 cursor-pointer transition">L</div>
                            </div>
                        </div>
                    </aside>
                </div>
