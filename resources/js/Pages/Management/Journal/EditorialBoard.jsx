            ...
            >
                ...
                <motion.div variants={itemVariants} className="bg-white/80 backdrop-blur-md p-8 rounded-[2rem] shadow-sm border border-slate-200/60 flex flex-col md:flex-row justify-between items-center gap-6">
                    <div className="flex items-center gap-5">
                        ...
                        <div>
                            <h1 className="text-2xl font-black text-slate-900 tracking-tight">Editorial Board</h1>
                            <p className="text-slate-500 font-medium">{journal.journal_title}</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-2 px-4 py-2 bg-blue-50 rounded-xl border border-blue-100">
                        <div className="w-2 h-2 rounded-full bg-blue-500 animate-pulse"></div>
                        <span className="text-[10px] font-black text-blue-700 uppercase tracking-widest">{members.length} Active Members</span>
                    </div>
                </motion.div>
                ...
                <div className="grid lg:grid-cols-3 gap-8">
                                ...
                                <table className="w-full text-left">
                                    <thead>
                                        <tr className="bg-slate-50/50 border-b border-slate-100">
                                            <th className="px-8 py-5 font-black text-slate-500 uppercase text-[10px] tracking-[0.2em]">Board Member</th>
                                            <th className="px-8 py-5 font-black text-slate-500 uppercase text-[10px] tracking-[0.2em]">Designation</th>
                                            ...
                                            <tr key={member.id} className="hover:bg-blue-50/30 transition-colors group">
                                                <td className="px-8 py-6">
                                                    <div className="flex items-center gap-4">
                                                        ...
                                                        <div>
                                                            <div className="font-black text-slate-900 group-hover:text-blue-700 transition-colors">{member.name}</div>
                                            ...
                                            <tr>
                                                <td colSpan="3" className="px-8 py-20 text-center">
                                                    <div className="flex flex-col items-center gap-4">
                                                        ...
                                                        <p className="text-slate-500 font-medium italic">No editorial board members assigned yet.</p>
                                                    </div>
                                                </td>
                                            </tr>
                                        )}
                    ...
                    {/* Add Member Form */}
                    <aside>
                        <motion.div variants={itemVariants} className="bg-white p-8 rounded-[2rem] shadow-xl border-2 border-blue-100 shadow-blue-500/5 sticky top-8">
                            ...
                            <form onSubmit={submit} className="space-y-5">
                                <InputGroup label="Full Name" icon={User} error={errors.name}>
                                    <input
                                        className="w-full bg-slate-50 border-slate-200 rounded-xl p-3 text-sm font-bold focus:ring-2 focus:ring-blue-500/20"
                                ...
                                <InputGroup label="Designation / Role" icon={Briefcase} error={errors.role}>
                                    <input
                                        className="w-full bg-slate-50 border-slate-200 rounded-xl p-3 text-sm font-bold focus:ring-2 focus:ring-blue-500/20"
                                ...
                                <button
                                    type="submit"
                                    disabled={processing}
                                    className="w-full bg-slate-900 text-white py-4 rounded-2xl font-black shadow-xl shadow-slate-900/20 hover:bg-blue-600 transition-all flex items-center justify-center gap-2 group mt-4"
                                >
