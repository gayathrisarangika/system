            ...
            >
                ...
                <motion.div variants={itemVariants} className="bg-white/80 backdrop-blur-md p-8 rounded-[2rem] shadow-sm border border-slate-200/60 flex flex-col md:flex-row justify-between items-center gap-6">
                    <div className="flex items-center gap-5">
                        ...
                        <div>
                            <h1 className="text-2xl font-black text-slate-900 tracking-tight">Symposium Articles</h1>
                ...
                <AnimatePresence mode="wait">
                    {(isFormVisible || editingArticle) && (
                        <motion.div
                            ...
                            className="bg-white rounded-[2.5rem] p-8 lg:p-12 border-2 border-purple-100 shadow-2xl shadow-purple-500/5"
                        >
                            <div className="flex items-center justify-between mb-10">
                                <div className="flex items-center gap-4">
                                    ...
                                    <h2 className="text-2xl font-black text-slate-900 tracking-tight">
                                        {editingArticle ? 'Modify Research Paper' : 'New Research Entry'}
                                    </h2>
                                </div>
                                <button
                                    onClick={() => {
                            ...
                            <form onSubmit={editingArticle ? submitEdit : submit} className="space-y-8">
                                <div className="grid md:grid-cols-2 gap-8">
                                    <InputGroup label="Paper Title" icon={Type} error={editingArticle ? editErrors.title : errors.title}>
                                        <textarea
                                            className="w-full bg-slate-50 border-slate-200 rounded-2xl p-4 text-sm font-bold focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all min-h-[100px]"
                                ...
                                <InputGroup label="Abstract" icon={LayoutList} error={editingArticle ? editErrors.abstract : errors.abstract}>
                                    <textarea
                                        className="w-full bg-slate-50 border-slate-200 rounded-2xl p-5 text-sm font-medium leading-relaxed focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all min-h-[200px]"
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                                    ...
                                    <InputGroup label="Page Numbers" icon={Search}>
                                        <input
                                            className="w-full bg-slate-50 border-slate-200 rounded-xl p-3 text-sm font-bold"
                                    ...
                                    <InputGroup label="Release Date" icon={Calendar}>
                                        <input
                                            type="date"
                                            className="w-full bg-slate-50 border-slate-200 rounded-xl p-3 text-sm font-bold"
                                ...
                                <div className="p-8 bg-purple-50/50 rounded-3xl border border-purple-100 flex flex-col md:flex-row items-center justify-between gap-6">
                                    <div className="flex items-center gap-4">
                                        <div className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center text-purple-600 shadow-sm">
                                            <FileUp size={24} />
                                        </div>
                                        <div>
                                            <p className="text-sm font-black text-slate-900">Digital Manuscript (PDF)</p>
                                            <p className="text-xs font-medium text-slate-500">Upload the final research file.</p>
                                        </div>
                                    </div>
                                    <div className="relative group">
                                        ...
                                        />
                                        <div className="bg-white px-6 py-3 rounded-xl border border-slate-200 font-bold text-xs group-hover:border-purple-400 group-hover:text-purple-600 transition-all flex items-center gap-2">
                                ...
                                <div className="flex justify-end gap-4">
                                    ...
                                    <button
                                        ...
                                        className="bg-slate-900 text-white px-10 py-4 rounded-2xl font-black shadow-xl shadow-slate-900/20 hover:bg-purple-600 transition-all flex items-center gap-2 group disabled:opacity-50"
                                    >
                                        <Save size={18} />
                                        <span>{editingArticle ? 'Commit Changes' : 'Publish Entry'}</span>
                                    </button>
                                </div>
                            </form>
                        </motion.div>
                ...
                <motion.div variants={itemVariants} className="space-y-6">
                    <div className="flex items-center justify-between px-4">
                        ...
                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">{articles.length} Manuscripts Found</span>
                    </div>
                    ...
                    <div className="grid gap-4">
                        {articles.length > 0 ? articles.map(article => (
                            <motion.div
                                key={article.id}
                                whileHover={{ y: -2 }}
                                className="bg-white p-6 rounded-3xl border border-slate-200/60 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-6 group hover:border-purple-500/30 hover:shadow-lg hover:shadow-purple-500/5 transition-all duration-300"
                            >
                                <div className="flex items-start gap-5 flex-1">
                                    <div className="w-12 h-12 rounded-2xl bg-slate-50 flex items-center justify-center text-slate-400 group-hover:bg-purple-50 group-hover:text-purple-600 transition-colors">
                                        <FileText size={24} />
                                    </div>
                                    <div>
                                        <h4 className="font-black text-slate-900 group-hover:text-purple-700 transition-colors leading-tight mb-1">{article.title}</h4>
                                        ...
                                        <div className="flex items-center gap-4 mt-3">
                                            ...
                                            {article.doi && <span className="text-[10px] font-black text-purple-400 bg-purple-50 px-2 py-0.5 rounded uppercase tracking-widest">DOI: {article.doi}</span>}
                                        </div>
                                    </div>
                                </div>
                                <div className="flex items-center gap-2 w-full md:w-auto self-end md:self-center">
                                    <button
                                        ...
                                        className="flex-1 md:flex-none flex items-center justify-center gap-2 px-5 py-2.5 bg-slate-50 text-slate-600 rounded-xl border border-slate-200 font-black text-xs uppercase tracking-widest hover:bg-purple-600 hover:text-white transition-all duration-300"
                                    >
                                        <Edit3 size={14} /> Edit
                                    </button>
                                    <a
                            ...
                            <div className="bg-white/50 border-2 border-dashed border-slate-200 rounded-[2.5rem] py-20 text-center">
                                ...
                                <p className="text-lg font-black text-slate-900">No entries yet</p>
                                <p className="text-slate-500 font-medium">Add papers to begin building your symposium collection.</p>
                            </div>
                        )}
                    </div>
                </motion.div>
