...
export default function Committee({ symposium, members }) {
    ...
    return (
        <BackendLayout title={`Committee - ${symposium.symposium_title}`}>
            ...
            <h1 className="text-2xl font-bold mb-8 text-slate-900">Committee - {symposium.symposium_title}</h1>
                        ...
                        {members.map(member => (
                            <tr key={member.id} className="border-b">
                                ...
                                <td className="p-4 text-right">
                                    <Link href={`/editor/symposium/committee/${member.id}`} method="delete" as="button" className="text-red-500">Delete</Link>
                                </td>
                            </tr>
                        ))}
                        {members.length === 0 && (
