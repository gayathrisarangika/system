                                ...
                                <table className="w-full text-left">
                                    <thead>
                                        <tr className="bg-slate-50/50 border-b border-slate-100">
                                            <th className="px-8 py-5 font-black text-slate-500 uppercase text-[10px] tracking-[0.2em]">Publication Identification</th>
                                    ...
                                    <tbody className="divide-y divide-slate-50">
                                        {abstractBooks.map(book => (
                                            <tr key={book.id} className="hover:bg-purple-50/30 transition-colors group">
                                                <td className="px-8 py-6">
                                                    <div className="flex items-center gap-4">
                                                        ...
                                                        <div>
                                                            <div className="font-black text-slate-900 group-hover:text-purple-700 transition-colors">{book.version}</div>
                    ...
                    <aside className="space-y-6">
                        <AnimatePresence mode="wait">
                            {isFormVisible && (
                                <motion.div
                                    ...
                                    className="bg-white p-8 rounded-[2rem] shadow-xl border-2 border-purple-100 shadow-purple-500/5 h-fit relative overflow-hidden"
                                >
                                        ...
                                        <form onSubmit={submit} className="space-y-5">
                                            <div className="space-y-1.5">
                                                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1 flex items-center gap-2">
                                                    <Calendar size={12} className="text-purple-500" /> Publication Year
                                                </label>
                                                <input type="number" className="w-full bg-slate-50 border-slate-200 rounded-xl p-3 text-sm font-bold focus:ring-2 focus:ring-purple-500/20" value={data.year} onChange={e => setData('year', e.target.value)} required />
                                            </div>

                                            <div className="space-y-1.5">
                                                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1 flex items-center gap-2">
                                                    <Type size={12} className="text-purple-500" /> Book Version
                                                </label>
                                                <input className="w-full bg-slate-50 border-slate-200 rounded-xl p-3 text-sm font-bold focus:ring-2 focus:ring-purple-500/20" value={data.version} onChange={e => setData('version', e.target.value)} required placeholder="e.g. Volume 1" />
                                            </div>

                                            <div className="space-y-1.5">
                                                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1 flex items-center gap-2">
                                                    <FileText size={12} className="text-purple-500" /> Digital Manuscript (PDF)
                                                </label>
                                                <div className="relative group">
                                                    ...
                                                    <div className="bg-slate-50 border-2 border-dashed border-slate-200 rounded-xl p-4 flex flex-col items-center justify-center gap-1 group-hover:border-purple-400 group-hover:bg-purple-50 transition-all">
                                                        <FileText size={20} className="text-slate-400 group-hover:text-purple-500" />
                                                        <span className="text-[10px] font-bold text-slate-500 uppercase tracking-tighter">Choose File</span>
                                                    </div>
                                                </div>
                                                {editingBook && <p className="text-[9px] text-slate-400 italic text-center mt-1">Leave blank to keep current</p>}
                                            </div>

                                            <div className="space-y-1.5">
                                                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1 flex items-center gap-2">
                                                    <ImageIcon size={12} className="text-purple-500" /> Cover Graphic
                                                </label>
                                                <div className="relative group">
                                                    ...
                                                    <div className="bg-slate-50 border-2 border-dashed border-slate-200 rounded-xl p-4 flex flex-col items-center justify-center gap-1 group-hover:border-purple-400 group-hover:bg-purple-50 transition-all">
                                                        <ImageIcon size={20} className="text-slate-400 group-hover:text-purple-500" />
                                            ...
                                            <div className="flex flex-col gap-3 pt-4">
                                                <button type="submit" disabled={processing} className="w-full bg-slate-900 text-white py-4 rounded-2xl font-black shadow-xl shadow-slate-900/20 hover:bg-purple-600 transition-all flex items-center justify-center gap-2 text-xs uppercase tracking-widest">
                                                    <Save size={16} />
                                                    {editingBook ? 'Commit Update' : 'Finalize Book'}
                                                </button>
                        ...
                        {!isFormVisible && (
                            <motion.div
                                initial={{ opacity: 0 }}
                                animate={{ opacity: 1 }}
                                className="bg-purple-50/50 border-2 border-dashed border-purple-200 rounded-[2rem] p-10 text-center"
                            >
                                <div className="w-16 h-16 bg-white rounded-2xl flex items-center justify-center text-purple-300 shadow-sm mx-auto mb-4">
                                    <BookOpen size={32} />
                                </div>
                                <p className="text-purple-800 font-black text-sm uppercase tracking-widest mb-1">Archive Management</p>
                                <p className="text-purple-600/70 text-xs font-medium leading-relaxed">
                                    Build your symposium repository by adding new abstract book volumes.
                                </p>
                            </motion.div>
                        )}
                    </aside>
